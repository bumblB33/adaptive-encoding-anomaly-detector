...
def _entry(detected_type: str, cardinality: int, missing: float, mutual_info: float):
...
def ieee_profile():
    ...
    for name, dtype, card, miss, mutual_info in _RULE_FIXTURE_COLUMNS:
        profile[name] = _entry(dtype, card, miss, mutual_info)
    for name, dtype, card, miss, mutual_info in _RESIDUAL_FIXTURE_COLUMNS:
        profile[name] = _entry(dtype, card, miss, mutual_info)
    return profile


def test_segment_labels_vocabulary_is_fixed_five():
    """The representation-geometry metrics bind to this vocabulary; freeze it."""
...
def test_save_load_roundtrip(ieee_profile, tmp_path):
    ...
    utils.save_to_json(assignments, out)
    assert out.exists()
    reloaded = utils.load_from_json(out)
